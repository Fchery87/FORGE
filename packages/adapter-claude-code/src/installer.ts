...
const DEFAULT_CLAUDE_MD_CONTENT = `# Forge Builder Contract
...
const DEFAULT_CLAUDE_COMMANDS: Record<string, string> = {
...
export async function install(targetDir: string, content: ClaudeCodeInstallContent = {}): Promise<void> {
  ...
  await Promise.all([
    writeFile(claudeMdPath, content.claudeMdContent ?? DEFAULT_CLAUDE_MD_CONTENT, 'utf8'),
    ...Object.entries(content.commands ?? DEFAULT_CLAUDE_COMMANDS).map(([fileName, body]) =>
