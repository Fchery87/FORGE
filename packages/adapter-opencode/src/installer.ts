...
const DEFAULT_OPENCODE_CONFIG_CONTENT = {
  forge: {
    role: 'builder',
...
const DEFAULT_OPENCODE_COMMANDS: Record<string, string> = {
...
export interface OpenCodeInstallContent {
  configContent?: Record<string, unknown>
  commands?: Record<string, string>
}

export async function install(targetDir: string, content: OpenCodeInstallContent = {}): Promise<void> {
  const configPath = join(targetDir, 'opencode.config.json')
  const commandsDir = join(targetDir, '.opencode', 'commands')
  await mkdir(commandsDir, { recursive: true })
  await Promise.all([
    writeFile(configPath, JSON.stringify(content.configContent ?? DEFAULT_OPENCODE_CONFIG_CONTENT, null, 2), 'utf8'),
    ...Object.entries(content.commands ?? DEFAULT_OPENCODE_COMMANDS).map(([fileName, body]) =>
